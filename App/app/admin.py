...
def send_email_function(program_id, subject, body):
    try:
        ...
        if not bookings:
            raise ValueError("No bookings found for this program.")
        ...
        if not email_list:
            raise ValueError("No valid email addresses found for this program.")
        ...
        flash(f"An unexpected error occurred while sending emails: {e}", 'danger')
        return False
...
def send_email():
    ...
    success = send_email_function(program_id, email_subject, email_body)
